...
public final class ContactsPresenter: NSObject {
    ...
    var contactBlock: ((ContactListViewModelProtocol, WalletAsset) -> Void)?
...
//            }
    ...
    private func switchViewModel(to state: ContactListState) {
        ...
        contactBlock?(viewModel, selectedAsset)
    }
...
extension ContactsPresenter: ContactViewModelDelegate {
    ...
    public func didSelect(contact: ContactViewModelProtocol, call: ScaleCodable) -> TransferViewController? {
        let receiveInfo = ReceiveInfo(accountId: contact.accountId,
                                      assetId: selectedAsset.identifier,
                                      amount: nil,
                                      details: nil)
        ...
        return coordinator.send(to: payload, call: call)
    }
}
