...
final class ContactsAssembly: ContactsAssemblyProtocol {
    ...
    static func assembleView(with resolver: ResolverProtocol, selectedAsset: WalletAsset, contactBlock: @escaping (ContactListViewModelProtocol, WalletAsset, ContactsPresenter) -> Void) -> ContactsViewProtocol? {
        let view = ContactsViewController(nibName: "ContactsViewController", bundle: Bundle(for: self))
        ...
        let presenter = ContactsPresenter(view: view,
                                          ...
                                          localSearchEngine: config.localSearchEngine,
                                          canFindItself: config.canFindItself)
        presenter.contactBlock = { (listViewModel, selectedAsset) in
            if listViewModel.numberOfSections > 0 {
                contactBlock(listViewModel, selectedAsset, presenter)
            }
        }
        view.presenter = presenter
