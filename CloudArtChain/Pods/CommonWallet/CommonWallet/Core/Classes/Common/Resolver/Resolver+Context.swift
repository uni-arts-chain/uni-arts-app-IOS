...
extension Resolver: CommonWalletContextProtocol {
    func createRootController() throws -> UIViewController {
...
//        return dashboardView.controller
        ...
        guard let accountListView = JLAccountListAssembly.assembleView(with: self, balanceBlock: nil) else {
            throw CommonWalletBuilderError.moduleCreationFailed
        }
        return accountListView.controller
    }
    
    func createRootController(balanceBlock: @escaping ([WalletViewModelProtocol]) -> Void) throws -> UIViewController {
        guard let accountListView = JLAccountListAssembly.assembleView(with: self, balanceBlock: balanceBlock) else {
            throw CommonWalletBuilderError.moduleCreationFailed
        }
        return accountListView.controller
    }
