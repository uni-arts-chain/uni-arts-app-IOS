...
extension SendCommand: WalletPresentationCommandProtocol {
    ...
    func getModelsExecute(contactBlock: @escaping (ContactListViewModelProtocol, WalletAsset, ContactsPresenter) -> Void) throws {
        ...
        self.contactsView = ContactsAssembly.assembleView(with: resolver, selectedAsset: asset, contactBlock: contactBlock) as! ContactsViewController
        try self.contactsView?.presenter.setup()
    }
