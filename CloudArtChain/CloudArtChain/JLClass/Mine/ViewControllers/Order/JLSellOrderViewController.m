...
@interface JLSellOrderViewController ()
@property (nonatomic, strong) NSArray *itemTitleArray;
@end

@implementation JLSellOrderViewController
- (id)init {
    self = [super init];
    if (self) {
        [self initTitleView];
...
- (NSArray*)itemTitleArray {
    if (!_itemTitleArray) {
        _itemTitleArray = @[@"全部订单", @"待发货", @"进行中", @"已完成"];
    }
...
- (__kindof UIViewController *)pageController:(WMPageController *)pageController viewControllerAtIndex:(NSInteger)index{
    JLSellOrderListViewController *orderListVC = [[JLSellOrderListViewController alloc] init];
