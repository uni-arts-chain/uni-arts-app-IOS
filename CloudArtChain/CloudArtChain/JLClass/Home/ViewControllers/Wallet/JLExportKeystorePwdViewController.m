...
- (UILabel *)noticeLabel {
    if (!_noticeLabel) {
        _noticeLabel = [JLUIFactory labelInitText:@"请设置导出Keystore密码，导入Keystore时需要校验！" font:kFontPingFangSCRegular(15.0f) textColor:JL_color_orange_FF8150 textAlignment:NSTextAlignmentCenter];
    }
    return _noticeLabel;
}
...
- (UILabel *)pwdTitleLabel {
    if (!_pwdTitleLabel) {
        _pwdTitleLabel = [JLUIFactory labelInitText:@"设置Keystore密码" font:kFontPingFangSCRegular(15.0f) textColor:JL_color_gray_212121 textAlignment:NSTextAlignmentLeft];
    }
...
- (UILabel *)confirmPwdTitleLabel {
    if (!_confirmPwdTitleLabel) {
        _confirmPwdTitleLabel = [JLUIFactory labelInitText:@"再次确认Keystore密码" font:kFontPingFangSCRegular(15.0f) textColor:JL_color_gray_212121 textAlignment:NSTextAlignmentLeft];
    }
...
- (UIButton *)nextButton {
    if (!_nextButton) {
        _nextButton = [JLUIFactory buttonInitTitle:@"下一步" titleColor:JL_color_white_ffffff backgroundColor:JL_color_blue_50C3FF font:kFontPingFangSCSCSemibold(18.0f) addTarget:self action:@selector(nextButtonClick)];
