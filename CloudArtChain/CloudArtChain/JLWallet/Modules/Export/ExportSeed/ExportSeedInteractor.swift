...
extension ExportSeedInteractor: ExportSeedInteractorInputProtocol {
    ...
    func fetchExportSeedDataForAddress(_ address: String, _ seedBlock: @escaping (String) -> Void) {
        let accountOperation = repository.fetchOperation(by: address, options: RepositoryFetchOptions())
            ...
            DispatchQueue.main.async {
                do {
                    ...
                    let viewModel = ExportStringViewModel(option: .seed,
                                                          ...
                                                          cryptoType: model.account.cryptoType,
                                                          data: model.seed.toHex(includePrefix: true))
                    seedBlock(viewModel.data)
                } catch {
                    self?.presenter.didReceive(error: error)
                }
            }
