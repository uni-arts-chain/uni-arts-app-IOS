//
//  JLBoxDetailCardCollectionWaterLayout.m
//  CloudArtChain
//
...
@interface JLBoxDetailCardCollectionWaterLayout()
...
@implementation JLBoxDetailCardCollectionWaterLayout
+ (instancetype)layoutWithColoumn:(int)coloumn data:(NSMutableArray *)dataA verticleMin:(float)minv horizonMin:(float)minh leftMargin:(float)leftMargin rightMargin:(float)rightMargin {
    JLBoxDetailCardCollectionWaterLayout *layout = [[JLBoxDetailCardCollectionWaterLayout alloc] init];
...
- (void)prepareLayout {
    ...
    self.sectionInset = UIEdgeInsetsMake(0.0f, 15.0f, 0, 15.0f);
    ...
    for(int i = 0; i < self.iconArray.count; i++) {
        ...
        //计算每个cell的高度
        float itemH = [self getcellHWithOriginSize:CGSizeMake(itemW, iconModel.imgHeight) itemW:itemW];
        //计算当前cell处于第几列
