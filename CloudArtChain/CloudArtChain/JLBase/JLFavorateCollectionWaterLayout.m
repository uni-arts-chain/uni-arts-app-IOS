//
//  JLFavorateCollectionWaterLayout.m
//  CloudArtChain
//
...
@interface JLFavorateCollectionWaterLayout()
...
@implementation JLFavorateCollectionWaterLayout
+ (instancetype)layoutWithColoumn:(int)coloumn data:(NSMutableArray *)dataA verticleMin:(float)minv horizonMin:(float)minh leftMargin:(float)leftMargin rightMargin:(float)rightMargin {
    JLFavorateCollectionWaterLayout *layout = [[JLFavorateCollectionWaterLayout alloc] init];
...
- (void)prepareLayout {
    ...
    for(int i = 0; i < self.iconArray.count; i++) {
        Model_members_favorate_arts_Data *facorateArtsData = self.iconArray[i];
        Model_art_Detail_Data *iconModel = facorateArtsData.favoritable;
