...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    ...
    //滚动结束判断
    if (![self hasViewScrolling]) {
        [self exceedEnableMaxDate];
    }
}
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    rowLabel.backgroundColor = JL_color_clear;
    rowLabel.frame = CGRectMake(0, 0, pickerView.bounds.size.width,self.frame.size.width);
    rowLabel.textAlignment = NSTextAlignmentCenter;
    rowLabel.font = [UIFont boldSystemFontOfSize:17];
    rowLabel.textColor = JL_color_black;
    ...
    if (pickerView == yearPicker) {
        rowLabel.text =  [NSString stringWithFormat:@"%@%@",self.yearArray[row],@"年"];
        return rowLabel;
    }else if(pickerView == monthPicker){
        ...
        if ([self.yearArray[yearRow] integerValue] == minYear) {
            ...
            rowLabel.text = [NSString stringWithFormat:@"%ld%@",(long)[self.minMonthRemainingArray[selectrow] integerValue],@"月"];
            ...
            rowLabel.text = [NSString stringWithFormat:@"%ld%@",(long)[self.maxMonthRemainingArray[selectrow] integerValue],@"月"];
        }else{
            rowLabel.text = [NSString stringWithFormat:@"%ld%@",row % 12 + 1,@"月"];
        }
        return rowLabel;
    }else{
        NSInteger yearRow = [yearPicker selectedRowInComponent:0] % self.yearArray.count;
        ...
        if ([self.yearArray[yearRow] integerValue] == minYear) {
            if ([self.minMonthRemainingArray[monthRow] integerValue] == minMonth) {
                ...
                rowLabel.text = [NSString stringWithFormat:@"%ld%@",(long)[self.minDayRemainingArray[selectrow] integerValue],@"日"];
            }else{
                ...
                rowLabel.text = [NSString stringWithFormat:@"%ld%@", row % monthRemainingDays + 1,@"日"];
            }
            ...
            if ([self.maxMonthRemainingArray[monthRow] integerValue] == maxMonth) {
                ...
                rowLabel.text = [NSString stringWithFormat:@"%ld%@",(long)[self.maxDayRemainingArray[selectrow] integerValue],@"日" ];
            }else{
            ...
            rowLabel.text = [NSString stringWithFormat:@"%ld%@", row % monthDays + 1,@"日"];
        }
        return rowLabel;
    }
}
...
#pragma mark 确定按钮点击
-(void)sureAction{
    NSString *dateStr = [self getSelectedDateString];
...
- (void)showWithMaxDateString:(NSString *)maxString withMinDateString:(NSString *)minString didSeletedDateStringBlock:(void (^)(NSString *dateString))dateStringBlock{
    ...
    //设置当前时间
    [self selectDate:now maxString:maxString minString:minString animated:NO];
    ...
    for (int i = 0 ; i <2; i++) {
        ...
        UIView * lineView = [[UIView alloc]initWithFrame:CGRectMake(0, (self.containerView.bounds.size.height-self.topView.bounds.size.height-48)/2.0+self.topView.bounds.size.height+i*48, kPICKVIEW_SCREEN_WIDTH,0.5)];
        lineView.backgroundColor = JL_color_gridLine;
    ...
    [self.topView setLZPickTopViewClickHandle:^(ClickTopViewType type){
        if (type==ClickTopViewTypeWithLeft) {
            ...
            //如果滚动中不能点击确认按钮
            if ([weakSelf hasViewScrolling]) {
                return ;
            }
            if (weakSelf.isAutoHiden) {
                [weakSelf hiddenWithAnimation];
            }
            [weakSelf sureAction];
        }
...
#pragma mark 设置选中时间
- (void)selectDate:(NSDate*)now maxString:(NSString*)maxString minString:(NSString*)minString animated:(BOOL)animated
{
    ...
    if ([self haveNull:maxString]  && [self haveNull:minString]) {
        //默认选中某个row
        [yearPicker selectRow:[dateComponent year]-[dateComponentTwo year] inComponent:0 animated:animated];
        [monthPicker selectRow:([dateComponent month]-[dateComponentTwo month]==0?0:[dateComponent month]-1) inComponent:0 animated:animated];
        [dayPicker selectRow: ([dateComponent day]-[dateComponentTwo day]==0?0:[dateComponent day]-1)   inComponent:0 animated:animated];
        [dayPicker reloadAllComponents];
    ...
    }else if (![self haveNull:maxString] )
    {
        if ([maxdateComponent year]-[dateComponent year]<=0) {
            ...
            if ([maxdateComponent year]-[dateComponent year]==0) {
                //默认选中某个row
                [yearPicker selectRow:self.yearArray.count-1 inComponent:0 animated:animated];
                [monthPicker selectRow:[dateComponent month]-1 inComponent:0 animated:animated];
                [dayPicker selectRow: [dateComponent day]-1   inComponent:0 animated:animated];
            }else{
                //最大时间 <当前时间
                [yearPicker selectRow:self.yearArray.count-1 inComponent:0 animated:animated];
                [monthPicker selectRow:[maxdateComponent month]-1 inComponent:0 animated:animated];
                [dayPicker selectRow: [maxdateComponent day]-1   inComponent:0 animated:animated];
            }
        }else{
            //最大时间>当前时间 设置滚动到当前时间
    ...
    }else if (![self haveNull:minString])
    {
        if ([dateComponent year]-[dateComponentTwo year]<=0) {
            ...
            if ([dateComponent year]-[dateComponentTwo year]==0) {
                //默认选中某个row
                [yearPicker selectRow:0 inComponent:0 animated:animated];
                [monthPicker selectRow:0 inComponent:0 animated:animated];
                [dayPicker selectRow: 0   inComponent:0 animated:animated];
            }else{
                //最小时间 >当前时间 设置滚动到当前时间
                [yearPicker selectRow:0 inComponent:0 animated:animated];
                [monthPicker selectRow:0 inComponent:0 animated:animated];
...
{
    [self selectDate:date maxString:nil minString:nil animated:animated];
}

- (BOOL)hasViewScrolling
{
    BOOL yearScroll  = [self anySubViewScrolling:yearPicker];
    BOOL monthScroll = [self anySubViewScrolling:monthPicker];
    BOOL dayScroll   = [self anySubViewScrolling:dayPicker];
    if (yearScroll || monthScroll || dayScroll) {
        return YES;
    }
    return NO;
}

#pragma mark 是否有视图在滚动
...
#pragma mark滚动超过最大时间
- (void)exceedEnableMaxDate
{
    if (self.unableExceedNow) {
        NSString *dateStr = [self getSelectedDateString];
        NSDate *selectedDate = [NSDate lz_dateWithString:dateStr];
        NSDate *now = [NSDate date];
        //超过当前时间,跳转到当前时间
        if ([selectedDate isLaterThan:now]) {
            [self setDate:now animated:YES];
        }
    }
}
