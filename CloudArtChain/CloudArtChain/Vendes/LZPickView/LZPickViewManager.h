...
//显示时间选择器
- (void)showWithDatePickerMode:(UIDatePickerMode)mode compltedBlock:(void (^)(NSDate *choseDate))compltedBlock cancelBlock:(void (^)(void))cancelBlock;
...
//设置当前选择
- (void)setCurrentSelectDate:(NSDate *)currentDate;
//显示地址三级联动选择器
- (void)showWithAddressPickerCompltedBlock:(void(^)(NSString *choseAddress,NSString *province , NSString *provinceID , NSString * city , NSString * cityId , NSString *area ,NSString *areaId))compltedBlock cancleBlock:(void(^)(void))cancelBlock;
//显示自定义单条，多条选择器
- (void)showMultiPickerViewWithData:(NSArray *)array withComponentCount:(int)componentCount withCompltedBlock:(void (^)(NSString *componentOneShow,NSString * componentOneBack,NSString *componentTwoShow,NSString * componentTwoBack,NSString *componentThreeShow,NSString * componentThreeBack))compltedBlock cancelBlock:(void (^)(void))cancelBlock;
//设置单条 显示的key、返回key
...
//是否自动隐藏 默认YES 设置日期选择框是否自动隐藏
- (void)setAutoHiden:(BOOL)isAutoHiden;
//隐藏日期
- (void)hiddenCustomLimitDatePicker;
