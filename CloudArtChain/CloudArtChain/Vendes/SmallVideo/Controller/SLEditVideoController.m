...
- (void)setupUI {
    ...
    self.preview.center = CGPointMake(self.view.sl_w / 2.0f, self.view.sl_h / 2.0f);
    [self.avPlayer play];
...
- (CALayer *)graffitiLayer {
    ...
    CGRect changeRect = CGRectMake(0.0f, 0.0f, CGRectGetWidth(graffitiLayer.frame)*scaleSize.width, CGRectGetHeight(graffitiLayer.frame)*scaleSize.height);
    ...
    [image drawInRect:CGRectMake(0.0f, 0.0f, self.avPlayer.naturalSize.width, self.avPlayer.naturalSize.height)];
...
- (NSMutableArray *)watermarkLayers {
    ...
    for (UIView *view in self.watermarkArray) {
        ...
        // 视频Layer上的坐标系原点在左下角，单位是px像素
        CGSize scaleSize = CGSizeMake(self.avPlayer.naturalSize.width / self.preview.sl_w, self.avPlayer.naturalSize.height / self.preview.sl_h);
        CGRect changeRect = CGRectMake(0.0f, 0.0f, CGRectGetWidth(animatedLayer.frame)*scaleSize.width, CGRectGetHeight(animatedLayer.frame)*scaleSize.height);
...
- (SLBlurView *)editBtn {
    if (_editBtn == nil) {
        _editBtn = [[SLBlurView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 70.0f, 70.0f)];
        _editBtn.center = CGPointMake(self.view.sl_w / 2.0f, self.view.sl_h - 80.0f);
        _editBtn.layer.cornerRadius = _editBtn.sl_w / 2.0f;
...
- (SLBlurView *)againShotBtn {
    if (_againShotBtn == nil) {
        _againShotBtn = [[SLBlurView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 70.0f, 70.0f)];
        _againShotBtn.center = CGPointMake((self.view.sl_w / 2.0f - 70.0f / 2.0f) / 2.0f, self.view.sl_h - 80.0f);
        _againShotBtn.layer.cornerRadius = _againShotBtn.sl_w / 2.0f;
...
- (UIButton *)saveAlbumBtn {
    if (_saveAlbumBtn == nil) {
        _saveAlbumBtn = [[UIButton alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 70.0f, 70.0f)];
        _saveAlbumBtn.center = CGPointMake(self.view.sl_w / 2.0f + 70.0f / 2.0f + (self.view.sl_w / 2.0f - 70.0f / 2.0f) / 2.0f, self.view.sl_h - 80.0f);
        _saveAlbumBtn.layer.cornerRadius = _saveAlbumBtn.sl_w / 2.0f;
...
- (UIButton *)doneEditBtn {
    if (_doneEditBtn == nil) {
        ...
        _doneEditBtn.backgroundColor = [UIColor colorWithRed:140.0f / 255.0f green:213.0f / 255.0f blue:255.0f / 255.0 alpha:1.0f];
        ...
        _editMenuView.selectEditMenu = ^(SLEditMenuType editMenuType, NSDictionary * _Nullable setting) {
            ...
            if (editMenuType == SLEditMenuTypeSticking) {
                ...
                if (image) {
                    ...
                    imageView.center = CGPointMake(weakSelf.preview.sl_w / 2.0f, weakSelf.preview.sl_h / 2.0f);
            ...
            if (editMenuType == SLEditMenuTypeText) {
                ...
                editTextView.editTextCompleted = ^(UILabel * _Nullable label) {
                    ...
                    label.center = CGPointMake(weakSelf.preview.sl_w / 2.0f, weakSelf.preview.sl_h / 2.0f);
            ...
            if(editMenuType == SLEditMenuTypeVideoClipping) {
                weakSelf.videoClippingView.exitClipping = ^{
                    ...
                    weakSelf.preview.center = CGPointMake(SL_kScreenWidth / 2.0f, SL_kScreenHeight / 2.0f);
                };
                [weakSelf hiddenEditMenus:YES];
                weakSelf.preview.transform = CGAffineTransformMakeScale((SL_kScreenWidth - 30.0f * 2) / SL_kScreenWidth, (SL_kScreenWidth - 30.0f * 2) / SL_kScreenWidth);
...
- (SLEditVideoClipping *)videoClippingView {
    if (!_videoClippingView) {
        _videoClippingView = [[SLEditVideoClipping alloc] initWithFrame:CGRectMake(0.0f, SL_kScreenHeight - 110.0f, SL_kScreenWidth, 110.0f)];
...
- (void)exportEditVideo {
    ...
    activityIndicatorView.color = [UIColor colorWithRed:140.0f / 255.0f green:213.0f / 255.0f blue:255.0f / 255.0 alpha:1];
    activityIndicatorView.center = CGPointMake(SL_kScreenWidth / 2.0f, SL_kScreenHeight / 2.0f);
...
- (void)dragAction:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        if (self.trashTips.center.y < rect.origin.y+rect.size.height / 2.0f) {
            ...
            pan.view.center = CGPointMake(self.preview.sl_w / 2.0f, self.preview.sl_h / 2.0f);
        }
