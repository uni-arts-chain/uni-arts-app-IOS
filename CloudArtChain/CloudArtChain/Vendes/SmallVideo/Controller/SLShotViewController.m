...
#define KMaxDurationOfVideo  15.0f //录制最大时长 s
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self focusAtPoint:CGPointMake(SL_kScreenWidth / 2.0f, SL_kScreenHeight / 2.0f)];
    //监听设备方向，旋转切换摄像头按钮
...
- (UIButton *)backBtn {
    if (_backBtn == nil) {
        ...
        _backBtn.frame = CGRectMake(0.0f, 0.0f, 30.0f, 30.0f);
        _backBtn.center = CGPointMake((self.view.sl_w / 2.0f - 70.0f / 2.0f) / 2.0f, self.view.sl_h - 80.0f);
        [_backBtn setImage:[UIImage imageNamed:@"hVideo_back"] forState:UIControlStateNormal];
...
- (UIView *)shotBtn {
    if (_shotBtn == nil) {
        ...
        _shotBtn.frame = CGRectMake(0.0f, 0.0f, 78.0f, 78.0f);
        _shotBtn.center = CGPointMake(self.view.sl_w / 2.0f, self.view.sl_h - 80.0f);
        _shotBtn.clipsToBounds = YES;
        _shotBtn.layer.cornerRadius = _shotBtn.sl_w / 2.0f;
        //轻触拍照，长按摄像
...
//        [_shotBtn addGestureRecognizer:longPress];
        //中心白色
        self.whiteView.frame = CGRectMake(0.0f, 0.0f, 57.0f, 57.0f);
        self.whiteView.center = CGPointMake(_shotBtn.sl_w / 2.0f, _shotBtn.sl_h / 2.0f);
        self.whiteView.layer.cornerRadius = self.whiteView.frame.size.width / 2.0f;
...
- (UIButton *)switchCameraBtn {
    if (_switchCameraBtn == nil) {
        _switchCameraBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.sl_w - 30.0f - 30.0f, 44.0f , 30.0f, 30.0f)];
        [_switchCameraBtn setImage:[UIImage imageNamed:@"btn_video_flip_camera"] forState:UIControlStateNormal];
...
- (CAShapeLayer *)progressLayer {
    if (_progressLayer == nil) {
        //设置画笔路径
        UIBezierPath *path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.shotBtn.frame.size.width / 2.0f, self.shotBtn.frame.size.height / 2.0f) radius:self.shotBtn.frame.size.width / 2.0f startAngle:- M_PI_2 endAngle:-M_PI_2 + M_PI * 2 clockwise:YES];
        //按照路径绘制圆环
        _progressLayer = [CAShapeLayer layer];
        _progressLayer.frame = _shotBtn.bounds;
        _progressLayer.fillColor = [UIColor clearColor].CGColor;
        _progressLayer.lineWidth = 10.0f;
        //线头的样式
        _progressLayer.lineCap = kCALineCapButt;
        //圆环颜色
        _progressLayer.strokeColor = [UIColor colorWithRed:140.0f / 255.0f green:213.0f / 255.0f blue:155.0f / 255.0 alpha:1.0f].CGColor;
...
- (SLShotFocusView *)focusView {
    if (_focusView == nil) {
        _focusView= [[SLShotFocusView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 60.0f, 60.0f)];
    }
    return _focusView;
}
...
- (UILabel *)tipsLabel {
    if (_tipsLabel == nil) {
        _tipsLabel = [[UILabel alloc] initWithFrame:CGRectMake((self.view.sl_w - 140.0f) / 2.0f, self.shotBtn.sl_y - 20.0f - 30.0f, 140.0f, 20.0f)];
...
- (void)startTimer {
    ...
    dispatch_source_set_timer(_gcdTimer, startDelayTime, timeInterval * NSEC_PER_SEC, 0.1f * NSEC_PER_SEC);
    /** 设置定时器任务
     * 可以通过block方式
     * 也可以通过C函数方式
     */
...
- (void)focusAtPoint:(CGPoint)point {
    ...
    self.focusView.transform = CGAffineTransformMakeScale(1.3f, 1.3f);
...
- (void)recordVideo:(UILongPressGestureRecognizer *)longPress {
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan: {
            self.shotBtn.sl_size = CGSizeMake(100.0f, 100.0f);
            self.shotBtn.center = CGPointMake(self.view.sl_w / 2.0f, self.view.sl_h - 80.0f);
            self.shotBtn.layer.cornerRadius = self.shotBtn.sl_h / 2.0f;
            self.whiteView.sl_size = CGSizeMake(40.0f, 40.0f);
            self.whiteView.center = CGPointMake(self.shotBtn.sl_w / 2.0f, self.shotBtn.sl_h / 2.0f);
            self.whiteView.layer.cornerRadius = self.whiteView.sl_w / 2.0f;
            //开始计时
            [self startTimer];
            //添加进度条
        ...
        case UIGestureRecognizerStateEnded: {
            self.shotBtn.sl_size = CGSizeMake(70.0f, 70.0f);
            self.shotBtn.center = CGPointMake(self.view.sl_w / 2.0f, self.view.sl_h - 80.0f);
            self.shotBtn.layer.cornerRadius =  self.shotBtn.sl_h / 2.0f;
            self.whiteView.sl_size = CGSizeMake(50.0f, 50.0f);
            self.whiteView.center = CGPointMake(self.shotBtn.sl_w / 2.0f, self.shotBtn.sl_h / 2.0f);
            self.whiteView.layer.cornerRadius = self.whiteView.sl_w / 2.0f;
            //取消计时器
            dispatch_source_cancel(self->_gcdTimer);
        ...
        [UIView animateWithDuration:0.3f animations:^{
            switch (deviceOrientation) {
                case UIDeviceOrientationPortrait:
                    self.switchCameraBtn.transform = CGAffineTransformMakeRotation(0.0f);
                    break;
                case UIDeviceOrientationLandscapeLeft:
                    self.switchCameraBtn.transform = CGAffineTransformMakeRotation(M_PI / 2.0f);
                    break;
...
- (void)captureTool:(SLAvCaptureTool *)captureTool didOutputPhoto:(UIImage *)image error:(NSError *)error {
    ...
    editViewController.saveToAlbum = YES;
    ...
    editViewController.againShotBlock = ^{
        [self.avCaptureTool startRunning];
    };
...
//    self.videoPath = outputFileURL;
    ...
    NSLog(@"视频文件大小 === %fM",fileSize / (1024.0f * 1024.0f));
}
@end
