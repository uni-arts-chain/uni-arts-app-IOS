...
#define KBottomMenuHeight 100.0f  //底部菜单高度
#define KGridTopMargin 40.0f  //顶部间距
#define KGridBottomMargin 20.0f  //底部间距
#define KGridLRMargin 20.0f   //左右边距
...
- (void)setupUI {
    ...
    if (newSize.height > self.maxGridRect.size.height) {
        ...
        self.zoomView.sl_centerX = self.view.sl_w / 2.0f;
    } else {
        self.zoomView.sl_size = newSize;
        self.zoomView.center = CGPointMake(self.view.sl_w / 2.0f, (self.view.sl_h - KBottomMenuHeight) / 2.0f);
    }
...
- (SLImageZoomView *)zoomView {
    if (!_zoomView) {
        ...
        _zoomView.sl_centerY = (self.view.sl_h - KBottomMenuHeight) / 2.0f;
...
- (UIButton *)rotateBtn {
    if (_rotateBtn == nil) {
//        _rotateBtn = [[UIButton alloc] initWithFrame:CGRectMake(30.0f, self.view.sl_h - KBottomMenuHeight, 40.0f, 30.0f)];
        _rotateBtn = [[UIButton alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 29.0f, 29.0f)];
        _rotateBtn.sl_centerX = self.view.sl_w / 2.0f;
        _rotateBtn.sl_centerY = self.cancleClipBtn.center.y;
...
//        _rotateBtn.titleLabel.font = [UIFont boldSystemFontOfSize:16.0f];
        [_rotateBtn setImage:[UIImage imageNamed:@"ic_rotation"] forState:UIControlStateNormal];
...
- (UIButton *)cancleClipBtn {
    if (_cancleClipBtn == nil) {
        _cancleClipBtn = [[UIButton alloc] initWithFrame:CGRectMake(0.0f, self.view.sl_h - 30.0f - 20.0f, 70.0f, 30.0f)];
//        [_cancleClipBtn setImage:[UIImage imageNamed:@"EditImageClipCancel"] forState:UIControlStateNormal];
//        _cancleClipBtn.titleLabel.font = [UIFont systemFontOfSize:14.0f];
        [_cancleClipBtn setTitle:@"取消" forState:UIControlStateNormal];
        [_cancleClipBtn setTitleColor:JL_color_white_ffffff forState:UIControlStateNormal];
        _cancleClipBtn.titleLabel.font = kFontPingFangSCRegular(16.0f);
...
- (UIButton *)recoveryBtn {
    if (_recoveryBtn == nil) {
        _recoveryBtn = [[UIButton alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 40.0f, 30.0f)];
        _recoveryBtn.sl_centerX = self.view.sl_w / 2.0f;
        ...
        _recoveryBtn.titleLabel.font = [UIFont boldSystemFontOfSize:16.0f];
...
- (UIButton *)doneClipBtn {
    if (_doneClipBtn == nil) {
        _doneClipBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.sl_w - 30.0f - 40.0f, self.view.sl_h - 30.0f - 20.0f, 70.0f, 30.0f)];
//        [_doneClipBtn setImage:[UIImage imageNamed:@"EditImageClipDone"] forState:UIControlStateNormal];
        [_doneClipBtn setTitle:@"完成" forState:UIControlStateNormal];
        [_doneClipBtn setTitleColor:JL_color_blue_38B2F1 forState:UIControlStateNormal];
        _doneClipBtn.titleLabel.font = kFontPingFangSCRegular(16.0f);
...
- (void)cancleClipClicked:(id)sender {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"sl_ImageClippingCancel" object:nil userInfo:nil];
}
