...
- (void)drawRect:(CGRect)rect {
    ...
    UIRectFill(rect);
    CGRect holeRectIntersection = CGRectIntersection(CGRectMake(self.leftTime.sl_x + 10.0f, 0.0f, self.rightTime.sl_x - (self.leftTime.sl_x + 10.0f), rect.size.height), rect);
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent*)event {
    ...
    //扩大响应区域宽至30
    CGFloat widthDelta = MAX(30.0f, 0.0f);
    CGFloat heightDelta = MAX(30.0f, 0.0f);
    bounds = CGRectInset(bounds, -0.5f * widthDelta, -0.5f * heightDelta);
    return CGRectContainsPoint(bounds, point);
}

- (void)setFrame:(CGRect)frame {
    [super setFrame:frame];
    self.boxInside.frame = CGRectMake(0.0f, 0.0f, self.sl_w, self.sl_h);
    self.leftTime.frame = CGRectMake(-10.0f, 0.0f, 10.0f, self.sl_h);
    self.rightTime.frame = CGRectMake(self.sl_w, 0.0f, 10.0f, self.sl_h);
}

#pragma mark - Getter
- (UIView *)boxInside {
    if (!_boxInside) {
        _boxInside = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, self.sl_w, self.sl_h)];
        _boxInside.backgroundColor = [UIColor clearColor];
        _boxInside.layer.borderColor = [UIColor whiteColor].CGColor;
        _boxInside.layer.borderWidth = 2.0f;
    }
    return _boxInside;
}
...
- (UILabel *)leftTime {
    if (!_leftTime) {
        _leftTime = [[UILabel alloc] initWithFrame:CGRectMake(-10.0f, 0.0f, 10.0f, self.sl_h)];
        _leftTime.text = @"||";
        ...
        _leftTime.font = [UIFont systemFontOfSize:12.0f];
...
- (UILabel *)rightTime {
    if (!_rightTime) {
        _rightTime = [[UILabel alloc] initWithFrame:CGRectMake(self.sl_w, 0.0f, 10.0f, self.sl_h)];
        _rightTime.text = @"||";
        _rightTime.tag = 1;
        ...
        _rightTime.font = [UIFont systemFontOfSize:12.0f];
...
- (UIView *)contentView {
    if (!_contentView) {
        _contentView = [[UIView alloc] initWithFrame:CGRectMake(40.0f, 10.0f, self.sl_w - 40.0f * 2, 50.0f)];
        ...
        _clippingBox.changeClippingRange = ^(SLVideoClippingState clippingState) {
            switch (clippingState.state) {
                case UIGestureRecognizerStateBegan:
                    weakSelf.contentView.layer.borderWidth = 2.0f;
                    break;
                ...
                case UIGestureRecognizerStateEnded:
                    weakSelf.contentView.layer.borderWidth = 0.0f;
                    if (clippingState.position) {
                        weakSelf.selectedClippingEnd(weakSelf.beginTime, weakSelf.endTime, clippingState.state);
                    } else {
                        weakSelf.selectedClippingBegin(weakSelf.beginTime, weakSelf.endTime,clippingState.state);
...
- (UIButton *)cancleBtn {
    if (_cancleBtn == nil) {
        _cancleBtn = [[UIButton alloc] initWithFrame:CGRectMake(10.0f, self.sl_h - 10.0f - 30.0f, 30.0f, 30.0f)];
...
- (UIButton *)doneBtn {
    if (_doneBtn == nil) {
        _doneBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.sl_w - 30.0f - 10.0f, self.sl_h - 10.0f - 30.0f, 30.0f, 30.0f)];
        [_doneBtn setTitle:@"完成" forState:UIControlStateNormal];
        [_doneBtn setTitleColor:[UIColor colorWithRed:45.0f / 255.0f green:175.0f / 255.0f blue:45.0f / 255.0f alpha:1.0f] forState:UIControlStateNormal];
