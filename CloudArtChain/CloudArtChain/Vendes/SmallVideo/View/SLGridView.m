...
#pragma mark - UI
- (void)setupUI {
    self.minGridSize = CGSizeMake(60.0f, 60.0f);
    self.maxGridRect = CGRectInset(self.bounds, 20.0f, 20.0f);
...
- (SLGridMaskLayer *)gridMaskLayer {
    if (!_gridMaskLayer) {
        ...
        _gridMaskLayer.maskColor = [UIColor colorWithWhite:0.0f alpha:0.6f].CGColor;
    }
...
- (SLGridLayer *)gridLayer {
    if (!_gridLayer) {
        ...
        _gridLayer.lineWidth = 2.0f;
        _gridLayer.gridColor = [UIColor whiteColor];
        _gridLayer.gridRect = CGRectInset(self.bounds, 20.0f, 20.0f);
    }
    return _gridLayer;
}
