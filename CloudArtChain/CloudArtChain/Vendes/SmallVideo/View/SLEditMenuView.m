...
- (void)createSubmenu {
    ...
    for (int i = 0; i < count; i++) {
        UIButton * colorBtn = [[UIButton alloc] initWithFrame:CGRectMake(space + (itemSize.width + space)*i, (self.frame.size.height - itemSize.height) / 2.0f, itemSize.width, itemSize.height)];
    ...
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0.0f, self.frame.size.height - 1.0f, self.frame.size.width, 0.5f)];
...
- (void)setImage:(NSString *)imageName {
    _imageView.frame = CGRectMake(0.0f, 0.0f, self.contentView.frame.size.width, self.contentView.frame.size.height);
...
- (UICollectionView *)collectionView {
    if (_collectionView == nil) {
        ...
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, self.frame.size.width, self.frame.size.height) collectionViewLayout:layout];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    return CGSizeMake((self.frame.size.width - 5 * 10.0f) / 4.0f, self.frame.size.height);
}
//列间距
...
- (void)createSubmenu {
    ...
    CGSize itemSize = CGSizeMake(50.0f, 50.0f);
    CGFloat space = (self.frame.size.width - count * itemSize.width) / (count + 1);
    for (int i = 0; i < count; i++) {
        UIButton * colorBtn = [[UIButton alloc] initWithFrame:CGRectMake(space + (itemSize.width + space)*i, (self.frame.size.height - itemSize.height) / 2.0f, itemSize.width, itemSize.height)];
        colorBtn.tag = 10 + i;
...
- (void)createEditMenus {
    ...
    for (int i = 0; i < count; i++) {
        UIButton * menuBtn = [[UIButton alloc] initWithFrame:CGRectMake(space / 2.0f + (itemSize.width + space) * i, self.frame.size.height - 80.0f, itemSize.width, 80.0f)];
...
- (void)defaultClipAction {
    [self menuBtnClicked:[self.menuBtns lastObject]];
}

#pragma mark - Getter
- (SLSubmenuGraffitiView *)submenuGraffiti {
    if (!_submenuGraffiti) {
        _submenuGraffiti = [[SLSubmenuGraffitiView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, self.frame.size.width, 60.0f)];
...
- (SLSubmenuStickingView *)submenuSticking {
    if (!_submenuSticking) {
        _submenuSticking = [[SLSubmenuStickingView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, self.frame.size.width, 60.0f)];
...
- (SLSubmenuMosaicView *)submenuMosaic {
    if (!_submenuMosaic) {
        _submenuMosaic = [[SLSubmenuMosaicView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, self.frame.size.width, 60.0f)];
