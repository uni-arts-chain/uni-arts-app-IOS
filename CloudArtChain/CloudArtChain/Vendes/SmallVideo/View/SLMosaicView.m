...
CGFloat angleBetweenPoints(CGPoint startPoint, CGPoint endPoint) {
    CGPoint Xpoint = CGPointMake(startPoint.x + 100.0f, startPoint.y);
    ...
    for (NSInteger i = 0; i < self.elementArray.count; i++) {
        ...
        if (blur.imageName) {
            ...
            if (image) {
                ...
                CGContextRef contextRef = CGBitmapContextCreate(nil, image.size.width, image.size.height, 8.0f, image.size.width * 4, colorRef, kCGImageAlphaPremultipliedFirst);
...
- (void)customInit {
    _squareWidth = 15.0f;
    _paintSize = CGSizeMake(50.0f, 50.0f);
