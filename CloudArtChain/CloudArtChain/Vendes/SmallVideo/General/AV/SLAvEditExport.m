    ...
    if (!_videoComposition) {
        ...
        switch (orientation) {
            case UIImageOrientationLeft:
                ...
                transform = CGAffineTransformTranslate(transform, 0.0f, assetVideoTrack.naturalSize.width);
                transform = CGAffineTransformRotate(transform,M_PI_2 * 3.0f);
                renderSize = CGSizeMake(assetVideoTrack.naturalSize.height,assetVideoTrack.naturalSize.width);
                break;
            case UIImageOrientationRight:
                //顺时针旋转90°
                //            NSLog(@"视频旋转90度,home按键在左");
                transform = CGAffineTransformTranslate(transform, assetVideoTrack.naturalSize.height, 0.0f);
    ...
    if (!_audioMix) {
        ...
        for (NSURL *audioUrl in self.audioUrls) {
            ...
            if (additional_assetAudioTrack) {
                ...
                [mixParameters setVolumeRampFromStartVolume:1.0f toEndVolume:0.3f timeRange:CMTimeRangeMake(kCMTimeZero, self.timeRange.duration)];
