...
- (void)overlayCreate
{
    if (_overlay == nil)
    {
        ...
        _overlay.translucent = YES;
        _overlay.backgroundColor = JL_color_clear;
        _overlay.layer.masksToBounds = YES;
    }
    if (_overlay.superview == nil) {
        if (_background == nil) {
            //CGRect backGroundRect=[_window convertRect:_window.bounds toView:(appdele)]
            ...
            _background.backgroundColor = JL_color_clear;
    ...
    if (_label == nil)
    {
        ...
        _label.backgroundColor = JL_color_clear;
...
- (void)overlayShow
{
    ...
    if (self.alpha == 0)
    {
        ...
        [UIView animateWithDuration:ANIMATION_DURATION delay:0 options:options animations:^{
            weakSelf.overlay.alpha = 1;
            weakSelf.background.alpha = 1;
