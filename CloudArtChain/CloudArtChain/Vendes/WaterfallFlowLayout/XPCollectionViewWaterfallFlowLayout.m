...
- (CGSize)collectionViewContentSize {
    ...
    //MAX(CGRectGetHeight(self.collectionView.bounds), _contentHeight);
    if ([self.dataSource respondsToSelector:@selector(collectionViewContentSizeChange:)]) {
         [self.dataSource collectionViewContentSizeChange:height];
    }
...
//    }];
    ...
    if (self.sectionHeadersPinToVisibleBounds) {
        for (UICollectionViewLayoutAttributes *attriture in result) {
            ...
            NSInteger numberOfItemsInSection = [self.collectionView numberOfItemsInSection:attriture.indexPath.section];
            if (numberOfItemsInSection == 0) continue;

            UIEdgeInsets contentInsetOfSection = [self contentInsetForSection:section];
            NSIndexPath *firstIndexPath = [NSIndexPath indexPathForItem:0 inSection:attriture.indexPath.section];
